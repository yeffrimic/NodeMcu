...
sk:on("receive", function(sck, payload) 
     ...
     while (day>dayspermonth[month]) do
          ...
          if (year % 4 < 1 and month == 2) then 
          day=day-1 
          end    -- february in leapyears
          month=month+1
     end
     --print(string.format("%02u-%02u-%04u",day,month,year))
     hora = string.format("%02u:%02u:%02u",hour+2,minute,second)
     print(hora.."  "..toint(day).."/"..month.."/"..toint(year))
     
     sck:close()
end )
sk:connect(123,"91.103.24.10")    -- GT-Guatemala
sk:send(request)
